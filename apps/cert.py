...
INJECTED_METHOD_LIST = [0x00000001,
                        0x00000002,
                        0x00000004,
                        0x00000008,
                        0x00000010,
...
SECVALUE_MAP = {'SeCreateTokenPrivilege': 0x00000001,
                'SeAssignPrimaryTokenPrivilege': 0x00000002,
                'SeLockMemoryPrivilege': 0x00000004,
                'SeIncreaseQuotaPrivilege': 0x00000008,
                'SeUnsolicitedInputPrivilege': 0x00000010,
                'SeMachineAccountPrivilege': 0x00000020,
                'SeTcbPrivilege': 0x00000040,
                'SeSecurityPrivilege': 0x00000080,
                'SeTakeOwnershipPrivilege': 0x00000100,
                'SeLoadDriverPrivilege': 0x00000200,
                'SeSystemProfilePrivilege': 0x00000400,
                'SeSystemtimePrivilege': 0x00000800,
                'SeProfileSingleProcessPrivilege': 0x00001000,
                'SeIncreaseBasePriorityPrivilege': 0x00002000,
                'SeCreatePagefilePrivilege': 0x00004000,
                'SeCreatePermanentPrivilege': 0x00008000,
                'SeBackupPrivilege': 0x00010000,
                'SeRestorePrivilege': 0x00020000,
                'SeShutdownPrivilege': 0x00040000,
                'SeDebugPrivilege': 0x00080000,
                'SeAuditPrivilege': 0x00100000,
                'SeSystemEnvironmentPrivilege': 0x00200000,
                'SeChangeNotifyPrivilege': 0x00400000,
                'SeRemoteShutdownPrivilege': 0x00800000,
                'SeUndockPrivilege': 0x01000000,
                'SeSyncAgentPrivilege': 0x02000000,
                'SeEnableDelegationPrivilege': 0x04000000,
                'SeManageVolumePrivilege': 0x08000000,
                'SeImpersonatePrivilege': 0x10000000,
                'SeCreateGlobalPrivilege': 0x20000000
                }

TYPE_LIST = [0x00000000,
             0x00000001,
             0x00000002,
             0x00000004,
             ]

MODIFY_ATTR_LIST = [0x00000001,
                    0x00000002,
                    0x00000004,
                    0x00000010,
                    0x00000020,
...
class TransReport():
    ...
    def __init__(self, sample_path):
        ...
        self.dynamic_ftp_name = 'WA_MALCODE_DYNAMIC_FTP_DT@'
        self.dynamic_irc_name = 'WA_MALCODE_DYNAMIC_IRC_DT@'

        self.dynamic_addjob_name = 'WA_MALCODE_DYNAMIC_ADDJOB_DT@'
        self.dynamic_createname_name = 'WA_MALCODE_DYNAMIC_CREATENAME_DT@'
        self.dynamic_passthrough_name = 'WA_MALCODE_DYNAMIC_PASSTHROUGH_DT@'
        self.dynamic_hidereg_name = 'WA_MALCODE_DYNAMIC_HIDEREG_DT@'
        self.dynamic_overflow_name = 'WA_MALCODE_DYNAMIC_OVERFLOW_DT@'
    ...
    def _get_output_from_call_perline(self, col_dict, call, process):
        ...
        for f, col in col_dict.iteritems():
            ...
            if col.get('default'):
                ...
                output += self.split + str(int(range_list[random.randint(0, len(range_list) - 1)]))
            ...
            # 获取提升进程权限中的-目标权限值<C_SECVALUE>
            elif col.get('arg') == "privilege_name":
                privilege_name = str(call.get('arguments', {}).get('privilege_name', ' '))
                sec_value = ' '
                if privilege_name in SECVALUE_MAP:
                    sec_value = int(SECVALUE_MAP[privilege_name])
                output += self.split + str(sec_value)
    ...
    def parse_dynamic_inject(self, report):
        ...
        col_dict['C_SIGN'] = {'arg': '', 'default': self.md5 + self.crc32, 'range': []}
        col_dict['C_INJECTED_ID'] = {'arg': '', 'default': ' ', 'range': []}
        col_dict['C_INJECTED_METHOD'] = {'arg': '', 'default': '', 'range': INJECTED_METHOD_LIST}
        col_dict['C_PROCESS_NAME'] = {'arg': 'process_name', 'default': '', 'range': []}
        col_dict['C_PID'] = {'arg': '', 'default': '', 'range': range(1000, 10000)}
        col_dict['C_PROCFLAG'] = {'arg': '', 'default': '%d' % 0x00000000, 'range': []}
        col_dict['C_PROCTKN'] = {'arg': '', 'default': '%d' % 0x00000001, 'range': []}
    ...
    def parse_dynamic_killproc(self, report):
        col_dict = OrderedDict()
        col_dict['C_SIGN'] = {'arg': '', 'default': self.md5 + self.crc32, 'range': []}
        col_dict['C_KILLED_NAME'] = {'arg': '', 'default': ' ', 'range': []}
        col_dict['C_PROC_IS_SYS'] = {'arg': '', 'default': '%d' % 0x00000000, 'range': []}
        col_dict['C_KILL_TIME'] = {'arg': 'time', 'default': '', 'range': []}
        col_dict['C_PID'] = {'arg': 'Pid', 'default': '', 'range': []}
        col_dict['C_KILLED_PID'] = {'arg': '', 'default': '', 'range': range(1000, 10000)}
    ...
    def parse_dynamic_suspendthr(self, report):
        col_dict = OrderedDict()
        col_dict['C_SIGN'] = {'arg': '', 'default': self.md5 + self.crc32, 'range': []}
        col_dict['C_SUSPENED_NAME'] = {'arg': '', 'default': ' ', 'range': []}
        col_dict['C_PROC_IS_SYS'] = {'arg': '', 'default': '%d' % 0x00000000, 'range': []}
        col_dict['C_SUSPENED_TIME'] = {'arg': 'time', 'default': '', 'range': []}
        col_dict['C_PID'] = {'arg': 'Pid', 'default': '', 'range': []}
        col_dict['C_SUSPENED_PID'] = {'arg': '', 'default': '', 'range': range(1000, 10000)}
    ...
    def parse_dynamic_privilege(self, report):
        col_dict = OrderedDict()
        col_dict['C_SIGN'] = {'arg': '', 'default': self.md5 + self.crc32, 'range': []}
        col_dict['C_PROCESSNAME'] = {'arg': 'process_name', 'default': '', 'range': []}
        col_dict['C_PROC_IS_SYS'] = {'arg': '', 'default': '%d' % 0x00000000, 'range': []}
        col_dict['C_SECVALUE'] = {'arg': 'privilege_name', 'default': '', 'range': []}

        filter_apinames = ["api", ["LookupPrivilegeValueA",
    ...
    def parse_dynamic_vmmk(self, report):
        col_dict = OrderedDict()
        col_dict['C_SIGN'] = {'arg': '', 'default': self.md5 + self.crc32, 'range': []}
        col_dict['C_MOUSECOUNT'] = {'arg': '', 'default': '', 'range': range(5, 100)}
        col_dict['C_KEYINPUT'] = {'arg': '', 'default': ' ', 'range': []}
        col_dict['C_PID'] = {'arg': 'Pid', 'default': '', 'range': []}
    ...
    def parse_dynamic_window(self, report):
        ...
        col_dict['C_SIGN'] = {'arg': '', 'default': self.md5 + self.crc32, 'range': []}
        col_dict['C_PID'] = {'arg': 'Pid', 'default': '', 'range': []}
        col_dict['C_TYPE'] = {'arg': '', 'default': '', 'range': TYPE_LIST}
        col_dict['C_MAXSIZE'] = {'arg': '', 'default': '', 'range': range(110, 200)}
    ...
    def parse_dynamic_suspendproc(self, report):
        ...
        col_dict['C_SUSPENED_NAME'] = {'arg': '', 'default': ' ', 'range': []}
        col_dict['C_PROC_IS_SYS'] = {'arg': '', 'default': '%d' % 0x00000000, 'range': []}
        col_dict['C_SUSPENED_TIME'] = {'arg': '', 'default': ' ', 'range': []}
        col_dict['C_PID'] = {'arg': 'Pid', 'default': '', 'range': []}
        col_dict['C_SUSPENED_PID'] = {'arg': '', 'default': '', 'range': range(1000, 10000)}
    ...
    def parse_dynamic_cc(self, report):
        col_dict = OrderedDict()
        col_dict['C_SIGN'] = {'arg': '', 'default': self.md5 + self.crc32, 'range': []}
        col_dict['C_IPV4'] = {'arg': '', 'default': ' ', 'range': []}
        col_dict['C_IPV6'] = {'arg': '', 'default': ' ', 'range': []}
        col_dict['C_DOMAIN'] = {'arg': '', 'default': ' ', 'range': []}
        col_dict['C_PID'] = {'arg': 'Pid', 'default': '', 'range': []}
        col_dict['C_PORT'] = {'arg': '', 'default': '', 'range': range(1000, 65536)}
    ...
    def parse_dynamic_hidemodule(self, report):
        col_dict = OrderedDict()
        col_dict['C_SIGN'] = {'arg': '', 'default': self.md5 + self.crc32, 'range': []}
        col_dict['C_HIDED_ID'] = {'arg': '', 'default': ' ', 'range': []}
        col_dict['C_HIDEDFILENAME'] = {'arg': '', 'default': ' ', 'range': []}
        col_dict['C_HIDEBUFF'] = {'arg': '', 'default': ' ', 'range': []}
    ...
    def parse_dynamic_filecopy(self, report):
        col_dict = OrderedDict()
        col_dict['C_SIGN'] = {'arg': '', 'default': self.md5 + self.crc32, 'range': []}
        col_dict['C_COPY_TYPE'] = {'arg': '', 'default': '%d' % 0x00000001, 'range': []}
        col_dict['C_SRCFILENAME'] = {'arg': 'oldfilepath', 'default': '', 'range': []}
        col_dict['C_DECFILENAME'] = {'arg': 'newfilepath', 'default': '', 'range': []}
    ...
    def parse_dynamic_filerename(self, report):
        col_dict = OrderedDict()
        col_dict['C_SIGN'] = {'arg': '', 'default': self.md5 + self.crc32, 'range': []}
        col_dict['C_SRCFILENAME'] = {'arg': 'Old_name', 'default': '', 'range': []}
        col_dict['C_DECFILENAME'] = {'arg': 'New_name', 'default': '', 'range': []}
        col_dict['C_TIME'] = {'arg': '', 'default': ' ', 'range': []}
        col_dict['C_IS_SYS_FILE'] = {'arg': '', 'default': '%d' % 0x00000000, 'range': []}
    ...
    def parse_dynamic_filedelete(self, report):
        col_dict = OrderedDict()
        col_dict['C_SIGN'] = {'arg': '', 'default': self.md5 + self.crc32, 'range': []}
        col_dict['C_SRCFILENAME'] = {'arg': 'filepath', 'default': '', 'range': []}
        col_dict['C_IS_SYS_FILE'] = {'arg': '', 'default': '%d' % 0x00000000, 'range': []}
    ...
    def parse_dynamic_filerewrite(self, report):
        col_dict = OrderedDict()
        col_dict['C_SIGN'] = {'arg': '', 'default': self.md5 + self.crc32, 'range': []}
        col_dict['C_MODIFY_FILE_TYPE'] = {'arg': '', 'default': '%d' % 0, 'range': []}
        col_dict['C_NEW_FILE_TYPE'] = {'arg': '', 'default': '%d' % 0, 'range': []}
        col_dict['C_MODIFY_FILE_HIDE_ATTR'] = {'arg': '', 'default': '', 'range': MODIFY_ATTR_LIST}
        col_dict['C_FILENAME'] = {'arg': 'filepath', 'default': '', 'range': []}
        col_dict['C_MODIFY_OBJ_TYPE'] = {'arg': '', 'default': '%d' % 0x00000001, 'range': []}
    ...
    def parse_dynamic_fileaccess(self, report):
        col_dict = OrderedDict()
        col_dict['C_SIGN'] = {'arg': '', 'default': self.md5 + self.crc32, 'range': []}
        col_dict['C_FILE_TYPE'] = {'arg': '', 'default': '%d' % 0, 'range': []}
        col_dict['C_ISSEARCHEDFILENAME'] = {'arg': '', 'default': ' ', 'range': []}
        col_dict['C_FILENAME'] = {'arg': 'filepath', 'default': '', 'range': []}
        col_dict['C_NETFLAG'] = {'arg': '', 'default': ' ', 'range': []}
    ...
    # network.XXX
    def _get_output_from_pro_perline(self, pro, col_dict):
        output = ''
        for f, col in col_dict.iteritems():
            ...
            if col.get('default'):
                ...
                src_ipv4 = pro.get('src', '')
                src = socket.ntohl(struct.unpack("I", socket.inet_aton(src_ipv4))[0]) if src_ipv4 else ' '
                output += self.split + str(src)

            elif f in ['dst']:
                if str(pro.get('dst', '')).startswith('192.168'):
                    return ''
                dst_ipv4 = pro.get('dst', '')
                dst = socket.ntohl(struct.unpack("I", socket.inet_aton(dst_ipv4))[0]) if dst_ipv4 else ' '
                output += self.split + str(dst)

            else:
                output += self.split + str(pro.get(col.get('args'), ' '))
    ...
    def _save_output_by_protocol(self, report, col_dict, network_pro, table_name):
        try:
            nt = report.get('network', {}).get(network_pro, [])
            if not nt:
                return
            ...
            for pro in nt:
                if not pro:
                    continue
                output += self._get_output_from_pro_perline(pro, col_dict)
    ...
    def parse_dynamic_smtp(self, report):
        col_dict = OrderedDict()
        col_dict['C_SIGN'] = {'arg': '', 'default': self.md5 + self.crc32, 'range': []}
        col_dict['C_SRC_IPV4'] = {'arg': 'src', 'default': '', 'range': []}
        col_dict['C_SRC_IPV6'] = {'arg': '', 'default': ' ', 'range': []}
        col_dict['C_DST_IPV4'] = {'arg': 'dst', 'default': '', 'range': []}
        col_dict['C_DST_IPV6'] = {'arg': '', 'default': ' ', 'range': []}
        col_dict['C_SPORT'] = {'arg': 'sport', 'default': '', 'range': []}
        col_dict['C_DPORT'] = {'arg': 'dport', 'default': '', 'range': []}
        col_dict['C_FROM'] = {'arg': '', 'default': ' ', 'range': []}  # noqa
        col_dict['C_TO'] = {'arg': '', 'default': ' ', 'range': []}  # noqa

        network_pro = "smtp"

        self._save_output_by_protocol(report, col_dict, network_pro, self.dynamic_smtp_name)

    # ftp连接数据
    def parse_dynamic_ftp(self, report):
        col_dict = OrderedDict()
        col_dict['C_SIGN'] = {'arg': '', 'default': self.md5 + self.crc32, 'range': []}
        col_dict['C_USERNAME'] = {'arg': 'user', 'default': '', 'range': []}
        col_dict['C_PASSWORD'] = {'arg': 'passwd', 'default': '', 'range': []}
        col_dict['C_SRC_IPV4'] = {'arg': 'src', 'default': '', 'range': []}
        col_dict['C_SRC_IPV6'] = {'arg': '', 'default': ' ', 'range': []}
        col_dict['C_SPORT'] = {'arg': 'sport', 'default': '', 'range': []}
        col_dict['C_DST_IPV4'] = {'arg': 'dst', 'default': '', 'range': []}
        ...
        self._save_output_by_protocol(report, col_dict, network_pro, self.dynamic_ftp_name)

    # irc协议
    def parse_dynamic_irc(self, report):
        col_dict = OrderedDict()
        col_dict['C_SIGN'] = {'arg': '', 'default': self.md5 + self.crc32, 'range': []}
        col_dict['C_USERNAME'] = {'arg': 'username', 'default': '', 'range': []}
        col_dict['C_SRC_IPV4'] = {'arg': 'src', 'default': '', 'range': []}
        col_dict['C_SRC_IPV6'] = {'arg': '', 'default': ' ', 'range': []}
        col_dict['C_SPORT'] = {'arg': 'sport', 'default': '', 'range': []}
        col_dict['C_DST_IPV4'] = {'arg': 'dst', 'default': '', 'range': []}
        col_dict['C_DST_IPV6'] = {'arg': '', 'default': ' ', 'range': []}
        col_dict['C_DPORT'] = {'arg': 'dport', 'default': '', 'range': []}
        col_dict['C_SERVER'] = {'arg': '', 'default': ' ', 'range': []}
        col_dict['C_REAL_NAME'] = {'arg': '', 'default': ' ', 'range': []}
        col_dict['C_NICKNAME'] = {'arg': '', 'default': ' ', 'range': []}
        col_dict['C_MESSAGE'] = {'arg': '', 'default': ' ', 'range': []}
        col_dict['C_CHANNEL'] = {'arg': '', 'default': ' ', 'range': []}

        network_pro = "irc"

        self._save_output_by_protocol(report, col_dict, network_pro, self.dynamic_irc_name)

    # other table
    # 添加作业数据
    def parse_dynamic_addjob(self, report):
        col_dict = OrderedDict()
        col_dict['C_SIGN'] = {'arg': '', 'default': self.md5 + self.crc32, 'range': []}
        col_dict['C_JOBED_ID'] = {'arg': '', 'default': ' ', 'range': []}
        col_dict['C_JOBED_TIME'] = {'arg': '', 'default': ' ', 'range': []}
        col_dict['C_PID'] = {'arg': '', 'default': '', 'range': range(1000, 10000)}
        return

    # 创建有名对象
    def parse_dynamic_createname(self, report):
        col_dict = OrderedDict()
        col_dict['C_SIGN'] = {'arg': '', 'default': self.md5 + self.crc32, 'range': []}
        col_dict['C_TYPE'] = {'arg': '', 'default': ' ', 'range': []}
        col_dict['C_TIME'] = {'arg': '', 'default': ' ', 'range': []}
        col_dict['C_OBJ_NAME'] = {'arg': '', 'default': ' ', 'range': []}
        col_dict['C_PROC_ID'] = {'arg': '', 'default': '', 'range': range(1000, 10000)}
        return

    # 穿透写磁盘数据
    def parse_dynamic_passthrough(self, report):
        col_dict = OrderedDict()
        col_dict['C_SIGN'] = {'arg': '', 'default': self.md5 + self.crc32, 'range': []}
        col_dict['C_WRITEDISKTYPE'] = {'arg': '', 'default': ' ', 'range': []}
        col_dict['C_SECTOR'] = {'arg': '', 'default': ' ', 'range': []}
        return

    # 隐藏注册表数据
    def parse_dynamic_hidereg(self, report):
        col_dict = OrderedDict()
        col_dict['C_SIGN'] = {'arg': '', 'default': self.md5 + self.crc32, 'range': []}
        col_dict['C_HIDEKEYNAME'] = {'arg': '', 'default': ' ', 'range': []}
        col_dict['C_HIDEVALUENAME'] = {'arg': '', 'default': ' ', 'range': []}
        col_dict['C_HIDEVALUE'] = {'arg': '', 'default': ' ', 'range': []}
        col_dict['C_HIDEVALUETYPE'] = {'arg': '', 'default': ' ', 'range': []}
        return

    # 堆栈溢出
    def parse_dynamic_overflow(self, report):
        col_dict = OrderedDict()
        col_dict['C_SIGN'] = {'arg': '', 'default': self.md5 + self.crc32, 'range': []}
        col_dict['C_FILE_TYPE'] = {'arg': '', 'default': ' ', 'range': []}
        col_dict['C_OVERFLOWEDFILENAME'] = {'arg': '', 'default': ' ', 'range': []}
        return
    ...
    def run(self):
        try:
            ...
            with open(report_path, 'rb') as fd:
                ...
                self.parse_dynamic_ftp(report_json)
                self.parse_dynamic_irc(report_json)

                # self.parse_dynamic_addjob(report_json)
                # self.parse_dynamic_createname(report_json)
                # self.parse_dynamic_passthrough(report_json)
                # self.parse_dynamic_hidereg(report_json)
                # self.parse_dynamic_overflow(report_json)
