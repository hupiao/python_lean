...
class SystemStatusManager(object):
    ...
    def read_mem_usage(cls):
        ...
        used = int(mem_info['MemUsed'] / GBYTES)
        free = int(mem_info['MemFree'] / GBYTES)
        logger.info(u'总内存：{0}GB，使用内存：{1}GB，剩余内存：{2}GB'.format(total, used, free))
